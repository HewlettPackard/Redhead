...
void open_data_shelves(int total_devices, int memory_allocate)
{
        int i, res;
        char buf[5];
...
debug("Allocating books from LFS for data shelves\n");
        ...
        for(i = 0; i < total_devices; i++)
        {
                res = ftruncate(shelf_descriptor[i], (int)memory_allocate/total_devices);
                if (res < 0)
                {
...
debug("Allocating memory for booksfrom LFS\n");
	...
	for(i = 0; i < total_devices; i++)
	{
               data_shelf[i] = (char *)mmap(0, (int)memory_allocate/total_devices, PROT_READ|PROT_WRITE, MAP_SHARED, shelf_descriptor[i], 0);
