...
#ifndef RAID6_TxHPC_H
#define RAID6_TxHPC_H
...
//#define DEBUG															/*Uncomment this flag to enable debug mode for TransHPC*/
...
easy data access across all the functions in TransHPC*/
	...
	extern char **data_region;                                                    //the anchor for entire data region in persistent memory
...
/*User facing functions that help in the use of TransHPC*/
	...
	char **initialize_raid6_parameters(int argc, char *argv[], char **shelves,
									   int all_devices);                    /*Initialize the RAID-6 parameters*/
	void run(void (*proc1)(void *, char **, meta *, int, char ***, char ***), void *args1, void(*proc2)(void *, meta *),
			 void *args2);    /*Execute the band gap mover and the functions passed to it*/
	void
	destroy_raid6_parameters();                                                /*Free the memory allocated for the devices*/
...
TransHPC*/
	...
	void
	print_stats();                                                        /*Print the statistics for encoding and decoding*/
	void erase(int shelf_id);                                                    /*Erase a device*/
	void
	erased_device(int shelf_id);                                                /*Keep track of the erased devices*/
	double calculate_time(clock_t begin, clock_t end);                                        /*Calculate the CPU time*/
	void *
	get_scratchpad();                                                    /*get the start anchor of the scratchpad*/
	void calculate_parity(
			char **ca_stripe);                                            /*Calculate the parity for the given data stripe*/
	void recover_device(
			char **re_stripe);                                                /*Recover a data device after erasures*/
	void
	initialize_jerasure();                                                    /*Initialize all the parameters and data structures required by Jerasure 2.0*/
	void initialize_meta(char *shelf,
						 long long unsigned pad);                                            /*Initialize the meta info*/
	void
	initialize_data_shelves(char **shelves);                                            /*Initialize the data shelves*/
	void initialize_data_stripes();                                                    /*Initialize the data stipes*/
	void set_free_band_gap(
			int begin);                                                /*Set the Free Band Gap on a particular stripe*/
	void create_data_region();                                                    /*Resolve and access data blocks*/
	size_t
	get_data_region_size();                                                    /*Give the size of the entire data region*/
	size_t get_stripesize();                                                    /*Give the size of each block*/
	int stripe_locator(
			int stripeid);                                                /*Locate the stripe with respect to the free band gap*/
	char **map_stripe(int stripeid);                                                /*Maps stripe to memory*/
	void update_stripe(
			int stripeid);                                                /*Update the stripe based on the free band gap movement*/
	void crash_recover(long long unsigned pad, char **shelves,
					   int all_devices);                                    /*Recover the TransHPC from a crash*/
