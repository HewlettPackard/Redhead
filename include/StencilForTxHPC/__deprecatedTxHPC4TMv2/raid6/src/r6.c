...
Check raid6/include/r6_utils.h for more info.*/
...
/*The following globals should not be changed
Starting here*/                                          
int *matrix;
int *bitmatrix;                    
int **smart_schedule;
...
char **data;
char **coding;
/*to here. They are used for parity calculations by Jerasure 2.0*/
/*This variable is for timing purposes*/
double tl;
...
void destroy_raid6_parameters()						/*Free the memory allocated for all TransHPC parameters*/
{
	...
	if(data_stripe == NULL || bitmatrix == NULL )
	{
...
double calculate_time(struct timespec begin, struct timespec end)	/*Calculate the time using CPU cycles*/	
{
	double total;
	total = ((double)((end.tv_sec*1.0e+9 + end.tv_nsec) - (begin.tv_sec*1.0e+9 + begin.tv_nsec))/1.0e+9);
	return total; 
}
...
debug("created datastripe\n");
	...
	for(i = 0; i < meta_info->no_of_stripes; i++)
	{
		...
		data_stripe[i].mapped_base = (char *)malloc(sizeof(char)*(meta_info->no_of_data_devices*meta_info->blocksize));
...
void set_free_band_gap(int begin)							/*Set the Free Band Gap to the begin index stripe. Only used for index 0 and (no of stripes - 1) at end of each step */
{
	int i;
...
#ifdef JERASURE		/*liberation codes*/
#ifdef DEBUG
...
#endif
		reed_sol_r6_encode(meta_info->no_of_data_devices, meta_info->wordsize, data, coding, sizeof(long));
#endif	
}
...
#endif
		jerasure_matrix_decode(meta_info->no_of_data_devices, meta_info->no_of_parity_devices, meta_info->wordsize, matrix, 1, meta_info->erasures, data, coding, sizeof(long));
#endif

}

void create_data_region()				/*Create the a consistent view of the data region at end of step*/
{/*Handled by calling framework, therefore commmented out here
	int i, j, k;
...
#ifdef JERASURE		/*Liberation codes*/
	bitmatrix = liberation_coding_bitmatrix(meta_info->no_of_data_devices, meta_info->wordsize);
#else			/*Reed Solomon codes*/

	matrix = reed_sol_r6_coding_matrix(meta_info->no_of_data_devices, meta_info->wordsize);
	bitmatrix = jerasure_matrix_to_bitmatrix(meta_info->no_of_data_devices, meta_info->no_of_parity_devices, meta_info->wordsize, matrix);  /*Creating a bitmatrix for the meta_info->no_of_data_devices and meta_info->wordsize*/ 
#endif
...
#endif			/*Not required by Reed-solomon codes*/

#ifdef DEBUG
debug("Schedule and Schedule Cache items created\n");
#endif
	if(!crashRecover)
	{
...
void initialize_raid6_parameters(int argc, char*argv[], char ** shelves, int all_devices)      /*Initialize the RAID-6 parameters*/
{
	...
	else
	{
		...
		if(sscanf(argv[1], "%llu", &meta_info->memory_allocate) == 0 || (meta_info->memory_allocate < 4096))                            /*Total memory for all devices except meta device*/
        	{
        	        error("USAGE: The memory to be allocated has to be a positive integer greater than 4096\n");
        	}
		meta_lfs->memory_allocate = meta_info->memory_allocate;
        	                                                                                                                             
		if(sscanf(argv[2], "%d", &meta_info->no_of_data_devices) == 0 || (meta_info->no_of_data_devices <= 4))                       /*No. of data devices*/                  
		{                                                                                                                                           
        		error("USAGE: The no. of devices have to be a positive integer greater than 4\n");                                              
		}    
		meta_lfs->no_of_data_devices = meta_info->no_of_data_devices;                                                                                                                                       
#ifndef JERASURE 		/*Liberation codes*/       	                                                                                                                                     
		if(sscanf(argv[3], "%llu", &meta_info->wordsize) == 0 || (meta_info->wordsize != 32 && meta_info->wordsize != 16 && meta_info->wordsize != 8 )) /*Word size for the code*/          
 		{                                                                                                                                           
 		       	error("USAGE: The word size should be a positive non-zero integer either 8, 16 or 32\n");                           
		}                                                                                                                                           
       		meta_lfs->wordsize = meta_info->wordsize;
#else				/*Reed-Solomon codes*/
                if(sscanf(argv[3], "%llu", &meta_info->wordsize) == 0 || (meta_info->wordsize < meta_info->no_of_data_devices ))             /*Word size for the code*/
                {
                        error("USAGE: The word size should be a positive prime non-zero integer greater than the number of devices\n");
                }
                meta_lfs->wordsize = meta_info->wordsize;
#endif
		...
		if(sscanf(argv[5], "%d", &meta_info->no_of_words_in_block) == 0 || (meta_info->no_of_words_in_block < 0))                    /*Number of words in every block*/
        	{
        	        error("USAGE: The number of words in a block should be a positive non-zero integer\n");
       		}
	
		if(sscanf(argv[6], "%d", &meta_info->prefetch_before) == 0 || (meta_info->prefetch_before < 0))                              /*Number of stripes to be prefetched before current stripe*/
        	{
        	        error("USAGE: The prefetch before should be a positive non-zero integer\n");
        	}
	
		if(sscanf(argv[7], "%d", &meta_info->prefetch_after) == 0 || (meta_info->prefetch_after < 0))                                /*Number of stripes to be prefetched after current stripes*/
        	{
...
#else			/*Reed-Solomon codes*/
		meta_info->devicewords = (unsigned long)((meta_info->memory_allocate)/(meta_info->wordsize*meta_info->total_devices)); 	     /*Number of words on each device*/
#endif
...
#else			/*Reed-Solomon codes*/
		meta_info->devicesize = meta_info->wordsize*meta_info->devicewords;		                                             /*Size of each device*/
                meta_lfs->devicesize = meta_info->devicesize;
#endif	
...
#else			/*Reed-Solomon codes*/
		meta_info->blocksize = meta_info->wordsize*meta_info->no_of_words_in_block;      		                             /*Size of each data block*/
                meta_lfs->blocksize = meta_info->blocksize;
#endif
...
debug("recovered data from device\n");
	...
	for(i = 0; i < meta_info->no_of_data_devices; i++)
        {
                memcpy(((char *)(data_stripe[stripeid].mapped_base) + (i*meta_info->blocksize)), temp_buf[i], meta_info->blocksize);
        }
#ifdef DEBUG
...
debug("Stripe located\n");
	...
	for(i = 0; i < meta_info->no_of_data_devices; i++)
        {
              	memcpy(temp_buf[i], ((char *)(data_stripe[stripeid].mapped_base) + (i*meta_info->blocksize)), meta_info->blocksize);
       	}
...
void run(void (*proc1)(void*, char*, meta*, int, char *, char **, char **), void* args1, void (*proc2)(void*, meta *), void* args2)		/*Execute the 2 procedures passed to run and move the free band gap*/
{
	int i, j;
	int stripe_order;
	int logical_stripe_id;
	char *cur_stripe;
	char *cur_stripe_load;
	char **before;
	char **after;
	...
	cur_stripe_load = (char *)malloc(sizeof(char)*meta_info->no_of_data_devices*meta_info->blocksize);
...
debug("prefetch pointers assigned\n");
	...
	while(!meta_info->end)																/*Run the loop till indicated to stop*/
	{
		...
		else
		{
			if(meta_info->counter%2 == 0)			
			{
				i = meta_info->free_band_gap_id;				/*Even step*/
			}
			else
			{
				i = meta_info->no_of_stripes - 1 - meta_info->free_band_gap_id;	/*Odd Step*/
			}
		}
#ifdef DEBUG
debug("stripe order set and starting bandgap loop\n");
		...
		while((i < (meta_info->no_of_stripes - meta_info->prefetch_after - meta_info->prefetch_before - 1)))					/*Move the free band gap*/
		{
			if(meta_info->counter%2 == 0)
         		{
                		stripe_order = i;					/*Even Step*/
				logical_stripe_id = stripe_order;
         		}
         		else
         		{
                		stripe_order = meta_info->no_of_stripes - i - 1;	/*Odd Step*/
				logical_stripe_id = stripe_order - (meta_info->prefetch_after + meta_info->prefetch_before + 1);
         		}
			if(i == 0)
			{
#ifdef DEBUG
debug("First stripe\n");
				...
				memcpy(cur_stripe_load, cur_stripe, (meta_info->no_of_data_devices*meta_info->blocksize));
				if(meta_info->counter%2 != 0 && meta_info->prefetch_before > 0)		/*Odd Step just prefetch before*/
				{
				...
				else if(meta_info->counter%2 == 0 && meta_info->prefetch_after > 0)	/*Even Step just prefetch after*/
				{
...
debug("Mid stripes in even step\n");
#endif
				if(meta_info->prefetch_before != 0)				/*Prefetch before swaps*/
				{
				...
				if(!crashRecover || ((meta_info->prefetch_after != 0) && !crashRecover))	/*Current Stripe*/
				{
					cur_stripe = after[0];
				}
				else
				...
				memcpy(cur_stripe_load, cur_stripe, (meta_info->no_of_data_devices*meta_info->blocksize));
				if(meta_info->prefetch_after != 0)						/*Prefetch after swaps*/
				{
					...
					if((stripe_order + j + 1) < (meta_info->no_of_stripes - meta_info->prefetch_after - meta_info->prefetch_before - 1))
					{
						after[j] = map_stripe(stripe_order + j + 1);
					}
					else
...
debug("Mid stripes in odd step\n");
				...
				if(!crashRecover || ((meta_info->prefetch_before != 0) && !crashRecover))		/*Current Stripe*/
				{
					cur_stripe = before[0];
				}
				else
				...
				if(meta_info->prefetch_before != 0)				/*Prefetch before swaps*/
				{
					...
					if((stripe_order - j - 1) > (meta_info->prefetch_after + meta_info->prefetch_before + 1))
					{                                                                                        
...
debug("Final stripe in odd step\n");
				...
				if((!crashRecover || ((meta_info->prefetch_before != 0) && !crashRecover)) && (before[0] != NULL))	/*Current Stripe*/
				{
 					cur_stripe = before[0];                                                                   
				}
				else
			...
			proc1(args1, cur_stripe, meta_lfs, logical_stripe_id, cur_stripe_load, before, after);						/*Call the subgrid function*/
#ifdef DEBUG	
