...
/*	matrix = (int *)malloc(sizeof(int)*meta_info->no_of_data_devices*meta_info->no_of_parity_devices);
	for(i = 0; i < meta_info->no_of_data_devices; i++)
	{
		matrix[i] = 1;
	}
	...
	for(i = 0; i < meta_info->no_of_data_devices; i++)
	{
		matrix[meta_info->no_of_data_devices + i] = j;
		j = galois_single_multiply(j, 32, meta_info->wordsize);
	}
*/
    printf("HERE no JERASURE DEFINED\n");
    fflush(NULL);
...
debug("Mid stripes in even step\n");
				...
				if(!crashRecover || ((meta_info->prefetch_after != 0) && !crashRecover))
				{
					cur_stripe = after[0];
				}
				else
...
debug("Mid stripes in odd step\n");
				...
				if(!crashRecover || ((meta_info->prefetch_before != 0) && !crashRecover))
				{
					cur_stripe = before[0];
				}
				else
...
debug("Final stripe in odd step\n");
			...
			proc1(args1, cur_stripe, meta_lfs, stripe_order, cur_stripe_load, before, after);						/*Call the subgrid function*/
#ifdef DEBUG	
