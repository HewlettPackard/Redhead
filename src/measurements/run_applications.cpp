...
#pragma mark CALL APPLICATIONS
    ...
    if(application == "jf"){
        ...
        double jacobian2 = jacobian2d(devshmPtr[0], 2500, 2500, true, iterations); //500 MB
        ...
        double jacTxHPC_measuredTime = jacobian2d_TxHPC((char**)devshmPtr, amountOfShelves, 200, 11180, 11180, iterations);
        std::cout << "Jacobi 2D TxHPC: " << jacTxHPC_measuredTime <<  " milliseconds" << std::endl;
        gnuplot_compareApplications("Jacobi2D_Iteration_ScalingUP", "Jacobian2D on /Dev/SHM - Size: XXXXX - Iterations: 1000", "_", "App_with_TxHPC", 0, jacTxHPC_measuredTime, "milliseconds");
    }

    else if(application == "gf"){
        std::cout << "gf started." << std::endl;
        //TODO FIX BUG: CODE FAILS FOR MORE THAN 2 ITERATIONS
        iterations = 3;
...
//        }
        ...
        //11200
        double gameOfLife_TxHPC_measured = gameOfLife_TxHPC_more_computation((char**) devshmPtr, amountOfShelves, 4, 11200, 11200, iterations+1);
        std::cout << "GameOfLife 2D TxHPC: " << gameOfLife_TxHPC_measured <<  " milliseconds" << std::endl;

        gnuplot_compareApplications("GameOfLife", "GameOfLife with 100 times more computation on /Dev/SHM - Size: ~500MB - Iterations: 2", "App_without_TxHPC", "App_with_TxHPC", gameOfLife1, gameOfLife_TxHPC_measured, "milliseconds");
    }
        ...
        std::cout << "SparseMatrix TxHPC: " << spM_TxHPC <<  " milliseconds" << std::endl;
        ...
        double accessTimes_TxHPC = access_TxHPC((char**) devshmPtr, amountOfShelves, 4, 2500, 2500, iterations+2);
        std::cout << "AccessTime: " << accessTimes_TxHPC <<  " milliseconds" << std::endl;

        double accessTime_withoutTxHPC = access_WithOutTxHPC ((char**) devshmPtr, 2500, 2500, iterations);
        std::cout << "AccessTime: " << accessTime_withoutTxHPC <<  " milliseconds" << std::endl;
        gnuplot_compareApplications("MemoryBandwidth", "Memory Bandwidth V2 on /Dev/SHM - Size: ~500MB - Iterations: 100", "App_without_TxHPC", "App_with_TxHPC", accessTimes_TxHPC, accessTime_withoutTxHPC, "microseconds");
    }
